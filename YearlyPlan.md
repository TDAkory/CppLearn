# Yearly Goal

## 2026

- O1 内功
  - KR1：数据库
    - 布局和格式 [Q1]从K、LevelDB、ClickHouse开始，利用excalidraw画内存布局和文件格式图，整理文档
    - 编码和压缩 [Q1]MIT公开课、论文调研，深入编码和压缩算法，[Q2]在K上进行实践
    - 论文学习 50篇 周更&整理笔记&做记录
  - KR2：高性能编程：
    - SIMD
    - Vector Search
    - Blog & CppCon Post
  - KR3：系统设计：
    - [Q1]s09g的Post、
  - KR4：AI & Agent
    - [Q1]上手练习AI Agent
- O2 外功（个人品牌计划）
  - KR1：博客（周更，年度50篇）
  - KR2：[Q2]软考：巩固基础知识 & 提前准备几篇论文，这次必须通过
  - 
- O3 探索
  - KR1：小说
    - [Q1]智能体+科幻 整一个
  - KR2：阅读
    - [Abseil Tips](https://abseil.io/tips/)
    - 内存连贯性和缓存一致性
    - Large-Scale Cpp
- O4 其他
  - KR1：健身
    - 恢复规律健身，注意强度，每周5练（单车、俯卧撑、杠铃操、核心、深蹲）
    - 检测体脂，每周测量
  - KR2：运动
    - 篮球：每周两打，保持运动习惯
    - 游泳
  - KR3：出游
    - 五一端午
    - 国庆
    - 27春节

## 2025

### 总结

- 博客：13/50，主要在知乎，达成率26%。26年加强数量和体系化，同时扩展平台范围
- 软考：考了两次都失败了，第一次基础没过，第二次论文没过。26年再试一次
- 读书：
  - 完成：`数据密集型应用系统设计`、[C++ 高性能编程（全）(译)](https://www.cnblogs.com/apachecn/p/18172912)、`程序员的自我修养-链接、装载与库`
  - 进行中：[Abseil C++ Tips of Week](https://abseil.io/tips/)、内存连贯性和缓存一致性导论、Large-Scale C++
- 系统设计放心的学习不够系统，进度不多
- 完成了装修 + 搬家，但是通勤比较累，作息不好。26年优化作息，晚上睡够8小时
- 忙于装修假期也没有出去玩。26年还是要规划出行，走走看看

### **需要完成的一些事情：**

| 事项                                     | 说明                                                 | 优先级                            | 进展                                        |
| -------------------------------------- | -------------------------------------------------- | ------------------------------ | ----------------------------------------- |
| [软考](https://www.ruankao.org.cn/guide) | 系统架构设计师 （3月底报名）5.25~5.28  （9月报名）11.9~12            | <font color=red>[P0]</font>    | <font color=green>[Done]</font> 遗憾没过，明年继续 |
| 金三银四                                   | 一轮interview，重新认识市场                                 | <font color=red>[P0]</font>    | <font color=yellow>[Pending]</font>       |
| [日语 N3](https://jlpt-main.neea.cn/)    | 7月12月考试，提前3个月报名                                    | <font color=orange>[P1]</font> | <font color=yellow>[Pending]</font>       |
| [中口](http://www.shwyky.net/portal/)    | 6月12月报名 3月9月中旬笔试 通过笔试的两年内报名口试（4次机会）4月10月报名，5月11月口试 | <font color=orange>[P1]</font> | <font color=yellow>[Pending]</font>       |
| 博客                                     | 发布50篇博客，构建自己的专栏                                    | <font color=red>[P0]</font>    | 4/50                                      |
| 代码                                     | StellarKit完善、Rust练手项目*3                            | <font color=red>[P0]</font>    |                                           |
### **Top Priority Tasks Q2**

| 事项                                                                              | 优先级                            | 进展                              |
| ------------------------------------------------------------------------------- | ------------------------------ | ------------------------------- |
| [Abseil C++ Tips of Week](https://abseil.io/tips/)                              | <font color=red>[P0]</font>    | <font color=blue>[Doing]</font> |
| CSAPP 跟着公开课再学一遍                                                                 | <font color=orange>[P1]</font> |                                 |
| 内存连贯性和缓存一致性导论                                                                   | <font color=red>[P0]</font>    |                                 |
| [软件设计的哲学-中译](https://cactus-proj.github.io/A-Philosophy-of-Software-Design-zh/) | <font color=red>[P0]</font>    |                                 |
| `数据密集型应用系统设计`                                                                   | <font color=red>[P0]</font>    | <font color=green>[Done]</font> |
| [C++ 高性能编程（全）(译)](https://www.cnblogs.com/apachecn/p/18172912)                  | <font color=red>[P0]</font>    | <font color=green>[Done]</font> |
| 学习 System Design : System Desing 101                                            | <font color=orange>[P1]</font> |                                 |
### **Top Priority Tasks Q1**

| 事项                 | 优先级 | 进展                              |
| ------------------ | --- | ------------------------------- |
| `程序员的自我修养-链接、装载与库` |     | <font color=green>[Done]</font> |

## 2024

> [To read](../Readings/ToBeRead.md)
>
> 在WAL + 共识 & 数据结构编码[内存&落盘] & 查询加速 & 二级索引+sorting key 等方向 选择一个 持续学习投入
> 
> 11、12 try some interview