...
Functional-Level Linking，把函数像模板一样单独保存在一个段里面，需要用到的才链接到输出文件中。好处是可以以减少输出文件的长度，坏处是增加编译和链接耗时

### 全局构造和析构

Linux系统的程序入口是 _start , 是glibc的一部分，负责程序的初始化，包括全局对象的构造等，完成动作后会调用main函数来执行。

在ELF中，定义了两个特殊的段来对应main函数前后的逻辑
...
要想把不同的编译器编译出来的目标文件链接到一起。必要的要求是：
...
我们把后三者（这些与可执行代码二进制兼容性相关的内容）称为ABI（Application Binary Interface）

影响ABI的因素非常多：硬件、编程语言、编译器、链接器、操作系统等。

对于C语言来说，以下几个方面决定目标文件之间是否二进制兼容：

* 内置类型的大小和在内存中的放置方式（大小端、内存对齐）
* 组合类型（struct、union、数组）的存储方式和内存分布
* 外部符号 与 用户定义符号之间的命名方式和解析方式
* 函数的调用方式（参数入栈顺序、返回值如何保持）
* 堆栈的分布方式（参数和局部变量在堆栈中的位置、参数的传递方式）
* 寄存器的使用约定（函数调用时，哪些寄存器允许修改，哪些必须保持）
* etc

对于C++而言，由于C++相较于C大大扩展了语言生态的特性支持，因此在二进制兼容上也有了更多的约束：

* 继承类体系的内存分布（基类、虚基类在继承类中的位置）
* 指向成员函数的指针的内存分布（如何调用、如何传递this）
* 如何调用虚函数、虚表的内容和分布、虚表指针在对象中的位置
* template如何实例化
* 外部符号的修饰
* 全局对象的构造和析构
* 异常的产生和捕获机制
...
一个静态库可以简单的看成是一组目标文件的集合，即很多目标恩家经过压缩打包后形成的一个文件。通常使用`ar`压缩一组目标文件产生，

1. 使用`ar -t xxx.a`可以查看一个静态库包含了哪些目标文件
2. 使用`ar -x xxx.a`可以将静态库解压成目标文件

要记住，静态库即便是一组目标文件压缩后的，仍然满足ELF文件格式，仍然可以利用 `objdump` `readelf` 来解析。

ld链接器可以自动寻找所有需要的符号及它们所在的目标文件，将这些目标文件从静态库中解压出来，最终链接成一个可执行文件。

## 链接过程控制

对于特殊的程序，比如操作系统内核、BIOS、驱动程序等，需要指定输出文件各个段的虚拟地址、段名、段存放的顺序等，这种情况下不能简单的使用链接器的默认配置来完成链接动作。
...
`ld -verbose`查看ld默认的链接脚本，`ld -T xxx.script`来指定输入一个链接脚本控制链接过程。
