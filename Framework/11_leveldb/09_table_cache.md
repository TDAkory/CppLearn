...
缓存的Key是 file_number，value是个名为 TableAndFile 的结构体，包含一个文件指针和一个table结构指针。
...
Status TableCache::Get(const ReadOptions& options, uint64_t file_number,
  ...
  if (s.ok()) {
    ...
    // 如果找到了就调用Table::InternalGet，这里调用了传入的函数指针
...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
  ...
  *handle = cache_->Lookup(key);
  if (*handle == nullptr) {
    // 若没有找到，则根据file number和db name打开一个RadomAccessFile
